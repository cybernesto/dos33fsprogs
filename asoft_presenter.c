...
void multiline_print(int *line_num, char *string, int cols) {
    int i, beg, end, len, tab;
    ...
    do{
        if(beg + cols < len)
            for(end = beg + cols; end > beg && string[end] != ' '; end--);
        else
            end = len;        
        
        printf("%d PRINT \"",*line_num);        
        for(i = 0; i < tab && beg != 0; i++)
            putchar(' ');               
        for(i=beg; i < end; i++)
            putchar(string[i]);
        printf("\"\n");
...
static void print_til_eof(FILE *fff,int *line_num, int scr_width) {
    ...
    while(1) {
        ...
        if ((string[0]=='%') && (string[1]=='c') && (string[2]=='%')) {
            center_print(*line_num,string+3, scr_width);
            (*line_num)++;
        } else {
            if(strlen(string) <= scr_width){
                printf("%d PRINT \"%s\"\n",*line_num,string);
                (*line_num)++;
            }
            else
                multiline_print(line_num, string, scr_width);
        }
    }
}
